...
df_hier_unscale <- data.frame("Expression" = seurat[["SCT"]]@scale.data[gene,], "Line" = seurat@meta.data$Assignment, "Time" = as.factor(seurat@meta.data$Pool), "Ncov" = seurat$Ncov)
colnames(df_hier_unscale)[1] <- "Expression"



##### Leave one out method #####
variables <- c("Line", "Time", "Ncov")
