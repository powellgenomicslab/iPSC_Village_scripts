...
outdir <- paste0(dir,"output/Seurat_w_Hash/Hashtags/")
outdir <- paste0(dir,"output/Seurat_w_Hash/Demultiplex_Doublet/")
...
if ( stage == "Hashtags"){
    ...
    dirs_hash <- paste0("/directflow/SCCGGroupShare/projects/DrewNeavin/iPSC_Village/data/Barcodes_200213_A00152_0206_AHK3HYDRXX/Hashing/", pools_hash, "/umi_count/")
    ...
    counts_list[1:6] <- lapply(names(counts_list)[1:6], function(x){
        print(x)
    ...
    ### Add hashtag data ###
    seurat_list_norm[1:6] <- lapply(names(seurat_list_norm)[1:6], function(x){
        seurat_list_norm[[x]][["HTO"]] <- CreateAssayObject(counts = hash_list[[x]])
        return(seurat_list_norm[[x]])
    })
    names(seurat_list_norm) <- pools

    seurat_list_norm[1:6] <- lapply(seurat_list_norm[1:6], function(x){
    ...
    lapply(seurat_list_norm[1:6], function(x){
        prop.table(table(x@meta.data$MULTI_ID))
    })
    ...
    ### Visualize results ###
    lapply(names(seurat_list_norm)[1:6], function(x){
    ...
    seurat_norm@meta.data$Site_rep[is.na(seurat_norm@meta.data$Site_rep)] <- "Sydney"
    ...
    p_Site_by_CellNumber <- ggplot(seurat_norm@meta.data, aes(factor(paste(seurat_norm@meta.data$MULTI_ID, seurat_norm@meta.data$Time), 
            levels = c("Brisbane1 Baseline","Brisbane2 Baseline","Brisbane3 Baseline", "Brisbane1 Day 4","Brisbane2 Day 4","Brisbane3 Day 4",
                        "Sydney1 Day 0","Sydney2 Day 0","Sydney3 Day 0", "Sydney1 Day 4","Sydney2 Day 4","Sydney3 Day 4", 
                        "Melbourne1 Day 0","Melbourne2 Day 0","Melbourne3 Day 0",
                        "Melbourne1 Day 4","Melbourne2 Day 4","Melbourne3 Day 4", 
                        "Doublet Day 0","Negative Day 0", "Doublet Day 4","Negative Day 4")), fill = seurat_norm@meta.data$Final_Assignment))) +
    ...
    p_Site_by_CellNumber <- ggplot(seurat_norm@meta.data, aes(factor(paste(seurat_norm@meta.data$MULTI_ID, seurat_norm@meta.data$Time), 
            levels = c("Brisbane1 Day_0","Brisbane2 Day_0","Brisbane3 Day_0", "Brisbane1 Day_4","Brisbane2 Day_4","Brisbane3 Day_4",
                        ...
                        "Doublet Day_0","Negative Day_0", "Doublet Day_4","Negative Day_4")), fill = factor(seurat_norm@meta.data$Final_Assignment, levels = c("FSA006","MBE1006","TOB421","doublet","unassigned","combination_fail_remove")))) +
