...
##### Set up variables #####
dir <- "/directflow/SCCGGroupShare/projects/DrewNeavin/iPSC_Village/output/eQTL_check/multi-passage/"


iPSC_Village/scripts/QCmetric_figs
##### Read in data #####
vcf <- read.vcfR(paste0(dir, "deboever_finalized_snps.vcf"))
...
for (gene in unique(eqtls_overlap_dt$gene_id)){
    ...
    while (k < nrow(temp[[gene]])){
        test_snp <- temp[[gene]]$ID[k]
        for (snp in temp[[gene]][(k+1):nrow(temp[[gene]]),]$ID){
            if (abs(cor(as.numeric(geno[ID == test_snp,1:(ncol(geno) - 1)]), as.numeric(geno[ID == snp,1:(ncol(geno) - 1)]))) == 1){
                temp[[gene]] <- temp[[gene]][ID != snp]
            } 
        }
        k = k + 1
    }
...
resid_files <- list.files("/directflow/SCCGGroupShare/projects/DrewNeavin/iPSC_Village/output/multi-passage/Variance/variance_integratedSCT/gene_separated/residuals4qtl")
...
eqtls_overlap_dt_subset_gene_snp <- eqtls_overlap_dt_subset[,c("gene_id","ID")]
