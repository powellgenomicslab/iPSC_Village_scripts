...
##### Set up variables #####
dir <- "/directflow/SCCGGroupShare/projects/DrewNeavin/iPSC_Village/output/eQTL_check/KilpinenOverlap/"


iPSC_Village/scripts/QCmetric_figs
##### Read in data #####
vcf <- read.vcfR(paste0(dir, "finalized_snps.recode.vcf"))
eqtls_overlap_dt <- fread(paste0(dir,"kilpinen_imputed_overlapping_filtered_header.bed"), sep = "\t")
...
resid_files <- list.files("/directflow/SCCGGroupShare/projects/DrewNeavin/iPSC_Village/output/variance_partition_post_review2/gene_separated/residuals4qtl")
ensg_list <- unique(gsub("_residuals4qtl.rds", "", resid_files))

eqtls_overlap_dt_subset <- eqtls_overlap_dt_subset[gene_id %in% ensg_list]


fwrite(eqtls_overlap_dt_subset, paste0(dir, "kilpinen_imputed_overlapping_filtered_header_pruned.bed"), sep = "\t")


eqtls_overlap_dt_subset_gene_snp <- eqtls_overlap_dt_subset[,c("gene_id","ID_ref_alt")]

fwrite(eqtls_overlap_dt_subset_gene_snp, paste0(dir, "kilpinen_imputed_overlapping_filtered_header_pruned_snp_gene.tsv"), sep = "\t")
