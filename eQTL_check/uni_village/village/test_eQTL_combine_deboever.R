...
xisq_df <- data.frame("DeBoever" = ifelse((results_dt$dataset_beta < 0), "negative", "positive"), "Dataset" = ifelse((results_dt$updated_beta < 0), "negative", "positive"))

xisq_results <- chisq.test(xisq_df$DeBoever, xisq_df$Dataset, correct=FALSE)
xisq_results$p.value
