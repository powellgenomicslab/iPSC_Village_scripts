...
##### Read in data #####
residuals <- readRDS(paste0("/directflow/SCCGGroupShare/projects/DrewNeavin/iPSC_Village/output/variance_partition_post_review2/village/gene_separated/residuals4qtl/", ensg, "_residuals4qtl.rds"))
residual_dt <- data.table(residuals)
colnames(residual_dt) <- c("residual")
residual_dt$Barcode <- names(residuals)

vcf <- read.vcfR("/directflow/SCCGGroupShare/projects/DrewNeavin/iPSC_Village/output/eQTL_check/uni_village/village/deboever_finalized_snps.recode.vcf")
