...
outdir <- paste0(dir,"output/scPred_Cluster_Assignments_Baseline_ref_Res0.2_7cluster/")
dir.create(outdir)


##### Read in Seurat Objects #####
seurat_brisbane_baseline <- readRDS(paste0(dir,"output/Brisbane_Clustering_Testing/Make_Brisbane_Baseline_Seurat_covs_res_0.2/Brisbane_Seurat_SCT_CellCycle_cov_Indiv_integration_w_meta.rds"))
...
##### Train the model #####
reference <- getFeatureSpace(seurat_brisbane_baseline, "Clusters_Res_0.2")
...
query_list <- lapply(seurat_list, function(x){
	...
	temp_list <- lapply(x, function(y) {
		if (length(y@meta.data$Pool) > 1){
			SCTransform(y, verbose = TRUE, vars.to.regress = c("scores.G1", "scores.S", "scores.G2M", "Pool", "percent.mt"), variable.features.n = 4000)
		} else {
			SCTransform(y, verbose = TRUE, vars.to.regress = c("scores.G1", "scores.S", "scores.G2M", "percent.mt"), variable.features.n = 4000)
		}
	})
...
prop_table_list <- lapply(names(query_list), function(x){
	temp <- as.data.frame(prop.table(table(query_list[[x]]$scpred_prediction)))
	colnames(temp) <- c("Cluster",x)
	return(temp)
})


prop_table_list[["Brisbane_Baseline"]] <- as.data.frame(prop.table(table(seurat_brisbane_baseline$Clusters_Res_0.2)))
colnames(prop_table_list[["Brisbane_Baseline"]]) <- c("Cluster","Brisbane_Baseline")


prop_table <- join_all(prop_table_list)
...
prop_table_long$Location_Time <- factor(prop_table_long$Location_Time, levels = c("Brisbane_Baseline", "Brisbane_Village_Day_4", "Melbourne_Baseline", "Melbourne_Village_Day_4", "Sydney_Baseline", "Sydney_Village_Day_4", "Sydney_Thawed_Village_Day_0", "Sydney_Thawed_Village_Day_7"))
