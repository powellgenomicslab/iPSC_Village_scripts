...
outdir <- paste0(dir,"output/scPred_Cluster_Assignments_Baseline_ref_Res0.13_5cluster/")
dir.create(outdir)
...
##### Train the model #####
reference <- getFeatureSpace(seurat_brisbane_baseline, "Clusters_Res_0.13")
...
query_list <- lapply(seurat_list, function(x){
	...
	temp_list <- lapply(x, function(y) {
		if (length(unique(y@meta.data$Pool)) > 1){
			SCTransform(y, verbose = TRUE, vars.to.regress = c("scores.G1", "scores.S", "scores.G2M", "Pool", "percent.mt"))
		} else {
