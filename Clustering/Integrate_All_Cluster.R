...
outdir <- paste0(dir,"output/All_data_integrated/")
dir.create(outdir)
...
##### Separate by cell line as well #####
seurat_subset_list <- list()
for (site_time in names(seurat_list)){
	for (cell_line in unique(seurat_list[[site_time]]@meta.data$Final_Assignment)){
		seurat_subset_list[[paste0(site_time,"_",cell_line)]] <- subset(seurat_list[[site_time]], subset = Final_Assignment == cell_line)
	}
}
...
cluster_meta <- data.frame(Clusters_Res_0.13 = seurat_brisbane_baseline@meta.data$Clusters_Res_0.13)
rownames(cluster_meta) <- rownames(seurat_brisbane_baseline@meta.data)


seurat_integrated <- AddMetaData(seurat_integrated, cluster_meta)
...
# seurat_features <- SelectIntegrationFeatures(object.list = seurat_subset_list, nfeatures = 3000)
# seurat_subset_list <- PrepSCTIntegration(object.list = seurat_subset_list, anchor.features = seurat_features)
# seurat_anchors <- FindIntegrationAnchors(object.list = seurat_subset_list, normalization.method = "SCT", anchor.features = seurat_features, reduction = "rpca")


# saveRDS(seurat_subset_list, paste0(outdir, "seurat_subset_list.rds"))
# saveRDS(seurat_features, paste0(outdir, "seurat_features.rds"))
# saveRDS(seurat_anchors, paste0(outdir, "seurat_anchors.rds"))

# seurat_subset_list <- readRDS(paste0(outdir, "seurat_subset_list.rds"))
# seurat_features <- readRDS(paste0(outdir, "seurat_features.rds"))
# seurat_anchors <- readRDS(paste0(outdir, "seurat_anchors.rds"))
...
saveRDS(seurat_integrated, paste0(outdir, "seurat_integrated_all_times.rds"))
seurat_integrated <- readRDS(paste0(outdir, "seurat_integrated_all_times.rds"))
...
Brisbane_clusters_pca <- DimPlot(seurat_integrated, group.by = "Clusters_Res_0.13", cells = rownames(seurat_integrated@meta.data)[which(!is.na(seurat_integrated@meta.data$Clusters_Res_0.13))])
...
pClustree <- clustree(ResolutionsDF[,1:35], prefix = "Resolution_")
...
resolutions <- c(0.02,0.03,0.05,0.07,0.08,0.12,0.13,0.16)
