...
df_hier_unscale <- data.frame("Expression" = seurat[["SCT"]]@scale.data[gene,], "Village" = as.factor(ifelse(seurat@meta.data$Time == "Baseline", 0, 1)), "Line" = seurat@meta.data$Final_Assignment, "Replicate" = as.factor(gsub("[A-Z][a-z]+", "", seurat@meta.data$MULTI_ID)), "Site" = seurat$Location)
colnames(df_hier_unscale)[1] <- "Expression"



##### Leave one out method #####
variables <- c("Line", "Village", "Site",  "Replicate","Village:Line", "Line:Site", "Village:Site", "Replicate:Village", "Replicate:Line", "Replicate:Site")
