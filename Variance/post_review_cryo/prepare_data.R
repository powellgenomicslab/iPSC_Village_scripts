...
seurat@meta.data$Cryopreserved <-ifelse(seurat@meta.data$Location == "Sydney_Cryopreserved", "Cryopreserved", "Fresh")
seurat@meta.data$Location <- gsub("_Cryopreserved", "", seurat@meta.data$Location)


seurat <- subset(seurat, subset = Location == "Sydney")
...
saveRDS(seurat, "/directflow/SCCGGroupShare/projects/DrewNeavin/iPSC_Village/output/variance_partition_post_review_cryo/seurat_integrated_Sydney_1pct_expressing.rds")

fwrite(data.table(Gene = rownames(seurat)), "/directflow/SCCGGroupShare/projects/DrewNeavin/iPSC_Village/output/variance_partition_post_review_cryo/seurat_integrated_Sydney_1pct_expressing_genes.tsv", sep = "\t")
