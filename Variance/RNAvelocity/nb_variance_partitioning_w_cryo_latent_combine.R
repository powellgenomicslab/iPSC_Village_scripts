...
seurat_dir <- "/directflow/SCCGGroupShare/projects/DrewNeavin/iPSC_Village/output/Variance/RNAvelocity/nb_partitioning_w_latent/data"
datadir <- "/directflow/SCCGGroupShare/projects/DrewNeavin/iPSC_Village/output/Variance/RNAvelocity/nb_partitioning_w_latent/gene_separated/"
outdir <- "/directflow/SCCGGroupShare/projects/DrewNeavin/iPSC_Village/output/Variance/RNAvelocity/nb_partitioning_w_latent/combined/"
dir.create(outdir)
...
if (!file.exists(paste0(outdir,"nb_variance_partitionin_df.tsv"))){
	##### Get list of files #####
	locations <- dir(datadir)[!(dir(datadir) %in% "logs")]


	fits_files <- lapply(locations, function(x){
		tmp <- list.files(paste0(datadir, x), pattern = "_variable_variance_partition_coefficients.rds")
	...
	##### Read in files #####
	fits_list <- lapply(names(fits_files), function(x){
		lapply(fits_files[[x]], function(y){
			readRDS(paste0(datadir,x,"/",y))
		})
	})
	names(fits_list) <- names(fits_files)

	fits <- lapply(names(fits_list), function(x){
		temp <- lapply(names(fits_list[[x]]), function(y){
			fits_list[[x]][[y]]$Location <- x
...
fits_df_long <- pivot_longer(fits_df, cols = c("Line", "Village", "Replicate","Pseudotime"), names_to = "Covariate", values_to = "Variance_Explained")
...
pVar_Explained_box <- ggplot(fits_df_long[which(fits_df_long$Location != "Site 2_Cryopreserved"),], aes(x = Covariate, y = Variance_Explained*100, color = Covariate)) +
	geom_boxplot(outlier.size = 0.5) +
	facet_grid(vars(Location)) +
	# scale_color_manual(values = variable_colors) +
	theme_classic() +
	ylab("Percent Gene Expression\nVariance Explained") 
	# geom_text_repel(size = 2, data = subset(fits_df_long[which(fits_df_long$Location != "Site 2_Cryopreserved"),], 
...
pCorr_Location_Point_Line <- ggplot(fits_df_joined[which(fits_df_joined$Covariate == "Line" & fits_df_joined$Location.x != "Site 2_Cryopreserved" & fits_df_joined$Location.y != "Site 2_Cryopreserved"),], aes(Variance_Explained.x*100, Variance_Explained.y*100, color = Covariate)) +
	geom_point(size = 0.5, alpha = 0.5) +
	facet_grid(rows = vars(Location.y), cols = vars(Location.x)) +
	scale_color_manual(values = variable_colors[["Line"]]) +
	theme_classic() +
	ylim(0,110)+
	geom_text(size = 2.5, x = 0, y = 110, aes(label = paste0("R^2 = ", Rsquared)), data = Rsquared[which(Rsquared$Covariate == "Line" & Rsquared$Location.x != "Site 2_Cryopreserved" & Rsquared$Location.y != "Site 2_Cryopreserved"),], hjust = 0) +
...
### CRYOPRESERVED ###
fits_df_long_cryo <- fits_df_long[grepl("Site 2", fits_df_long$Location),]
fits_df_long_cryo$Location <- gsub("Site 2_Cryopreserved", "Cryopreserved", fits_df_long_cryo$Location) %>% gsub("Site 2", "Fresh", .)
...
pVar_Explained_box_Cryo <- ggplot(fits_df_long_cryo, aes(x = Covariate, y = Variance_Explained*100, color = Covariate)) +
	...
	geom_text_repel(size = 2.5, data = subset(fits_df_long_cryo, 
							((Gene_ID %in% c("CHCHD2", "TBL1Y") & Covariate == "Line") | 
...
pCorr_Location_Point_Line_cryo <- ggplot(fits_df_joined_cryo[which(fits_df_joined_cryo$Covariate == "Line"),], aes(Variance_Explained.x*100, Variance_Explained.y*100, color = Covariate)) +
	...
	theme_classic() +
	ylim(0,110)+
	geom_text(size = 3, x = 0, y = 110, aes(label = paste0("R^2 = ", Rsquared)), data = Rsquared_cryo[which(Rsquared_cryo$Covariate == "Line"),], hjust = 0) +
...
pCorr_Location_Point_Village_cryo <- ggplot(fits_df_joined_cryo[which(fits_df_joined_cryo$Covariate == "Village"),], aes(Variance_Explained.x*100, Variance_Explained.y*100, color = Covariate)) +
	...
	theme_classic() +
	ylim(0,70)+
	geom_text(size = 3, x = 0, y = 70, aes(label = paste0("R^2 = ", Rsquared)), data = Rsquared_cryo[which(Rsquared_cryo$Covariate == "Village"),], hjust = 0) +
	xlab("Percent Gene Expression\nVariance Explained - Fresh") +
	ylab("Percent Gene Expression\nVariance Explained - Cryopreserved") +
