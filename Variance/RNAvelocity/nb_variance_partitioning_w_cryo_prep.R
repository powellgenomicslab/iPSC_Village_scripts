...
outdir <- paste0(dir,"output/Variance/RNAvelocity/nb_partitioning/data/")
...
seurat_noNA@meta.data$Location_Quintile <- paste0(seurat_noNA@meta.data$Location, "_", seurat_noNA@meta.data$Quintile)



seurat_sub <- list()
for (location in unique(seurat_noNA@meta.data$Location_Quintile)){
	seurat_sub[[location]] <- subset(seurat_noNA, subset = Location_Quintile == location)
}
