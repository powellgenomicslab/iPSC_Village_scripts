...
SCT <- readRDS(paste0(datadir, location,"_SCT_seurat_1pct.rds"))
...
if (number <= nrow(SCT[["SCT"]]@counts)){
	...
	df <- data.frame("Expression" = data.frame(SCT[["SCT"]]@counts[gene,]), "Line" = SCT@meta.data$Final_Assignment, "Village" = ifelse(SCT@meta.data$Time == "Baseline", 0, 1), "Replicate" = gsub("[A-Z][a-z]+", "", SCT@meta.data$MULTI_ID), "Pseudotime" = SCT@meta.data$latent_time) ### The results are not dependent on the order or the covariates included so don't need to include extra covariates
	colnames(df)[1] <- "Expression"


	### Fit models
	model <- glmmTMB(Expression ~ 1 + (1|Line) + (1|Village) + (1|Replicate) + (1|Pseudotime), data = df, family = nbinom2) ### The results are not dependent on the order or the covariates included so don't need to include extra covariates
	...
	saveRDS(variance_partition, paste0(outdir, location, "_", gene, "_variable_variance_partition_coefficients.rds"))
